...
inline float dBox(float3 pos, float3 size){
    float3 d = abs(pos) - size;
    return length(max(d, .0)) 
        + min(max(d.x, max(d.y, d.z)), .0);
}

inline float dRoundBox(float3 pos, float3 size, float roundness){
    float3 d = abs(pos) - size;
    return length(max(d, .0)) - roundness
        + min(max(d.x, max(d.y, d.z)), .0);
}

inline float dTorus(float3 pos, float radius, float thickness){
    float2 q = float2(length(pos.xz) - radius, pos.y);
    return length(q) - thickness;
}

//inline float dCappedTorus(float3 pos){
...
//}
//
//inline float dRoundCone(float3 pos){
//    float2 q = float2(length(pos.xz), pos.y);
//
...
//}
//
inline float dPlane(float3 pos, float3 dir)
{
    return dot(pos, dir);
}
//
//// 動かない
//inline float dQuad(float3 pos){
//    float3 a = _QuadA.xyz;
//    float3 b = _QuadB.xyz;
//    float3 c = _QuadC.xyz;
//    float3 d = _QuadD.xyz;
...
inline float dHexPrism(float3 pos, float2 h){
    pos = abs(pos);
    return max(pos.z - h.y, max((pos.x * .866025 + pos.y * .5), pos.y) - h.x);
}

//inline float dTriangle(float3 pos){
//    float2 h = float2(_TriangleX, _TriangleY);
//    float3 q = abs(pos);
//    return max(q.z - h.y, max(q.x * .866025 + pos.y * .5, -pos.y) -h.x * .5);
//}
//
//inline float dOcta(float3 pos){
...
//}
//
//inline float dCapsule(float3 pos){
