Shader "Raymarching/AnimationPrac"
{
    Properties
    {
        ...
        [PowerSlider(2.5)] _RotateSpeed("Speed", Range(1., 200)) = 1.
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 rotate(float3 pos, float angle){
                ...
                if(abs(axis.x) <= EPS && abs(axis.y) <= EPS && abs(axis.z) <= EPS) return pos;
            ...
            float dHexPrism(float3 pos, float2 h){
                pos = abs(pos);
                return max(pos.z - h.y, max((pos.x * .866025 + pos.y * .5), pos.y) - h.x);
            }
            ...
            float map(float3 pos){
                float depth = .5;
                pos.z = mod(pos.z, depth * 2.) - .5 * depth * 2.;
                float hex1 = dHexPrism(rotate(pos, radians(_CosTime.w * _RotateSpeed)));
//                pos.z = mod(pos.z, depth * 1.9) - .5 * depth * 1.9;
                float hex2 = dHexPrism(rotate(pos, radians(2. * _SinTime.z * _RotateSpeed)), float2(_HexagonX * .7, _HexagonY * 1.01 ));

                float hex3 = dHexPrism(rotate(pos, radians(2. * _SinTime.z * _CosTime.w * _RotateSpeed)), float2(_HexagonX * .5, _HexagonY * 1.02 ));

                float hex4 = dHexPrism(rotate(pos, radians(3. * _SinTime.z * _CosTime.w * _SinTime.w * _RotateSpeed)), float2(_HexagonX * .3, _HexagonY * 1.03 ));
                
                return opSubstruct(hex1, opSubstruct(hex2, opSubstruct(hex3, hex4)));
            }
